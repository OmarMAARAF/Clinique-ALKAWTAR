...
(function(t) { "function" == typeof define && define.amd ? define(["jquery"], t) : t(jQuery) })(function(t) {
    t.ui = t.ui || {}, t.ui.version = "1.12.1", t.ui.keyCode = { BACKSPACE: 8, COMMA: 188, DELETE: 46, DOWN: 40, END: 35, ENTER: 13, ESCAPE: 27, HOME: 36, LEFT: 37, PAGE_DOWN: 34, PAGE_UP: 33, PERIOD: 190, RIGHT: 39, SPACE: 32, TAB: 9, UP: 38 };
    var e = 0,
...
HTMLimages = document.querySelectorAll(".news-image");
HTMLtitles = document.querySelectorAll(".news-title");
HTMLdates = document.querySelectorAll(".news-date");
HTMLdescription = document.querySelectorAll(".news-description");
HTMLAuthors = document.querySelectorAll(".post__meta-author");
HTMLSources = document.querySelectorAll(".post__meta-cat");
HTMLurl = document.querySelectorAll(".news-link");
...
fetch("https://current-news.p.rapidapi.com/news/health", {
    ...
    .then(data => {
        ...
        urls = [];
        for (i = 0; i < 6; i++) {
            titles[i] = data.news[i].title;
            images[i] = data.news[i].urlToImage;
            dates[i] = data.news[i].publishedAt;
            description[i] = data.news[i].description;
            content[i] = data.news[i].content;
            authors[i] = data.news[i].author;
            sources[i] = data.news[i].source['name'];
            urls[i] = data.news[i].url;
        }
        console.log(HTMLurl)

        for (i = 0; i < 6; i++) {
            HTMLimages[i].src = images[i];
            HTMLtitles[i].innerHTML = titles[i];
            if (HTMLtitles[i].innerText.length > 40) {
                HTMLtitles[i].innerText = HTMLtitles[i].innerText.substring(0, 40) + '...';
            }
            HTMLdates[i].innerText = dates[i].substring(0, 10);
            HTMLdescription[i].innerHTML = description[i];
            HTMLAuthors[i].innerText = authors[i];
            HTMLSources[i].innerText = sources[i];
            HTMLurl[i].href = urls[i];

        }
        //Set the DOM elements from our API
